...
export function createReceptionistTools(ctx: ToolContext) {
  const { store, companyId, conversationId, aiService, executed } = ctx;
  ...
  const sendConfirmationEmail = tool(
    ...
    },
    {
      ...
      description: 'Send a confirmation email after booking an appointment, when you know the visitor email address.',
      schema: z.object({
